...
def DBExists(file,sheet,row,column,conn):
    allSql = getArray(file,sheet,row,column[7],column[8]) + \
             getArray(file,sheet,row,column[9],column[10]) + \
             getArray(file,sheet,row,column[16],column[19])
    '''
...
def sqlExcept(file,sheet,row,column,userParams,userParamsValue,userVar,userVarValue,conn):
    ...
    msg = ['数据库异常']
    SqlMsg = []
    sqlArray = getArray(file,sheet,row,column[7],column[8]) + \
               getArray(file,sheet,row,column[9],column[10]) + \
               getArray(file,sheet,row,column[18],column[19])
