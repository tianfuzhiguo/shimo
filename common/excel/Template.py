...
class Template(Init):
    ...
    def verTemp(self, sheetName, sheet, bookRes, sheetRes, fileRes):
        ...
        if '未找到关键字' in str(msg):
            ...
            ss = "['关键字顺序不正确'," + "'" + str(self.getValue(fileRes, sheet, 1, msg[1])) + "','" + str(
                self.getValue(fileRes, sheet, 1, msg[2])) + "']"
            ...
            if '数量不一致' in str(info):
                ...
                for i in range(1, len(info)):
                    if fileRes.endswith('.xls'):
                        ...
                        self.setValueColor(sheetRes, 2, int(info[i]), self.getValue(fileRes, sheet, 1, int(info[i])),
                                           "blue")
                bookRes.save(fileRes)
                self.consoleFunc('green', sheetName + ':', 'size=4')
    ...
    def verKeyWordExist(self, file, sheet):
        ...
        # 定义关键字数组
        arr = ['name', 'url', 'method', 'param', 'file', 'header', 'part101', 'part201', 'part301', 'section101',
               'section201',
               'section301', 'resText', 'resHeader', 'statusCode', 'expression', 'status', 'time', 'init001',
               'restore001', 'dyparam001',
