...
class Http(Init,analy):
    ...
    '''
    @接口请求超时时间目前配的是30S,个人觉得已经足够长了，再重试3次是否有必要？
    @先注释掉，30S超时后不再重新请求
    '''
...
#     res.mount('https://',HTTPAdapter(max_retries=3))
    ...
    def httpRequest(self,file,sheetName,userParams,userParamsValue,sheet,row,conn,column):
        ...
        url,method,pay,files,header=[repAll(str(getValue(file,sheet,row,column[i])),file,sheet,row,conn,column,userParams,userParamsValue,self.userVar,self.userVarValue) for i in range(5)]
        '''
